...
open class MovieWriter: Object {
    ...
    /// Called when the audio speaker mode used for recording the audio is requested by the engine. This can affect the number of output channels in the resulting audio file/stream. Defaults to ``AudioServer/SpeakerMode/stereo`` if ``_getAudioSpeakerMode()`` is not overridden.
    @_documentation(visibility: public)
    ...
    public static func addWriter (_ writer: MovieWriter?) {
        ...
        gi.object_method_bind_ptrcall_v (MovieWriter.method_add_writer, nil, nil, &copy_writer_handle)
        
        #else
        
        var _args: [UnsafeRawPointer?] = []
        withUnsafePointer (to: writer?.handle) { p0 in
        _args.append (writer == nil ? nil : p0)
        
            gi.object_method_bind_ptrcall (MovieWriter.method_add_writer, nil, &_args, nil)
        }
